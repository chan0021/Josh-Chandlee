...
namespace Review_Loops_Collections_Classes
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] studentLastNames = { "Harris", "Ackerman", "Smith" };
            studentIds[0] = 1;
            studentIds[1] = 2;
            studentIds[2] = 3;
            ...
            foreach (var student in studs)
            {
                ...
                bool isUpper = true;
                string crazyFirsName = "";
                foreach (var letter in student.FirstName)
                {
                    if (isUpper == true)
                    {
                        crazyFirsName += letter.ToString().ToUpper();
                        isUpper = false;

                    }
                    else
                    {
                        crazyFirsName += letter.ToString().ToLower();
                        isUpper = true;
                    }
                }
                Console.WriteLine($"Crazy firstname is {crazyFirsName}");
            }
